...
\begin{lemma}
  ...
  object in $\cat{C}$ and $\Hom{C}{A}{0}$ is non-empty, then $A \cong
  0 \times A$.
\end{lemma}
\begin{proof}
  Suppose $f \in \Hom{C}{A}{0}$.  We can define two maps using products:
  \[
  \begin{array}{lll}
    A \mto^{\langle f , \id_{A} \rangle} 0 \times A\\
    A \times 0 \mto^{\pi_1} A
  \end{array}
  \]
  By the universal mapping property for products $\langle f , \id_{A}
  \rangle;\pi_1 = \id_{A}$ and by the previous lemma $\pi_1;\langle f
  , \id_{A} \rangle = \id_{A \times 0}$, because $A \times 0$ is
  initial.  This then implies that because
\end{proof}
\noindent
