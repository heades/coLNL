...
TND takes off from Crolard~s paper \emph{A formulae-as-types interpretation of subtractive logic}
JLC 2004, where he gives a term assignment to bi-intuitionisitc logic within a variant of 
Parigot's~\cite{Parigot:1992} classical $\lambda\mu$-calculus and interprets the term assigned to 
subtraction rules in terms of coroutines. He then restricts the calculus He then restricts the calculus
to provide a constructive version of it ( \emph{safe coroutines}).. The second author has then used 
a variant of Crolard's calculus as a term assignment to co-intuitionistic logic and to linear 
co-intuitionistic logic \cite{Bellin:2012} without using  the $\lambda\mu$-calculus. In this formulation, 
the reduction of a term in context may impact other terms in the context.
...
\begin{remark}
 ...
 Hence, we choose to leave the typing of $p$ terms implicit in
