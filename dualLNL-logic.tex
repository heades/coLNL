...
formalization, then a natural deduction formalization, and finally a
term assignment to the natural deduction version.  Each of these
systems will consistently use the same syntax and naming conventions
for formulas, types, and contexts given by the following definition.
\begin{definition}
...
\begin{figure}
  \begin{mdframed}
    ...
    \end{mathpar}
  \end{mdframed}
  \caption{Non-linear fragment of the DLNL sequent calculus}
...
\begin{figure}
  \begin{mdframed}
    ...
    \end{mathpar}
  \end{mdframed}
  \caption{Linear fragment of the DLNL sequent calculus}
...
\end{lemma}
\begin{proof}
  By induction on $d(\Pi_1) + d(\Pi_2)$.  We consider only give one
  cast where the last inferences of $\Pi_1$ and $\Pi_2$ are logical
  inferences; please see
...
In DLNL logic contexts are treated multiplicatively. Some cases of the
previous proof would be best treated with additive contexts, for
...
\begin{array}{cllllll}
  \text{(non-linear terms)} & [[s]],[[t]] & ::= & [[x]] \mid [[connectW to t]] \mid [[t1 * t2]] \mid
       ...
       [[H e]] \mid [[let J x = e in t]] \mid [[let H x = t1 in t2]] \mid \\
       & & & [[postp(x -> t1,t2)]]\\
       \\
...
\begin{remark}
  Let us call terms of the form $[[postp(x -> t1,t2)]]$,
  $[[postp (x -> e1,e2)]]$, and $[[postpP e]]$ $p$-\emph{terms}. Let us say
  ...
  typed with $0$.  In presence of the $\bot$ rule and of the 0-rule it
  is possible to replace a non-$p$-normal $p$-term removed by a
  $\beta$-reduction with a $\bot$ or 0-rule. However this creates an
...
\begin{center}
  \begin{tabular}{rcl}
    ...
    $FV(\mathtt{casel}\; e)$ &\ =\ & $FV(e)$\\
    $FV(\mathtt{caser}\; e)$ &\ =\ & $FV(e)$\\
  \end{tabular}
\end{center}
...
\begin{center}
   \begin{tabular}{rcl}
$\Psi, \Pi$ \qquad & ::= & $\cdot \mid t : T \mid \Psi, \Pi$\\
$\Gamma, \Delta$\qquad&  ::= & $\cdot \mid e: A \mid \Gamma, \Gamma'$\\
\end{tabular}
\end{center}
We also use the notation 
\[
\Delta = e_1: A_1\,\|\, \cdots \,\|\, e_n: A_n\qquad \hbox{and}\qquad \Psi = t_1: S_1, \ldots, t_n: S_n
\]
...
\begin{figure}
  \begin{mdframed}
    ...
    \end{mathpar}
  \end{mdframed}
  \caption{Non-linear fragment of the term assignment for \textbf{co-ND}}
...
\begin{figure}
  \begin{mdframed}
    ...
    \end{mathpar}
  \end{mdframed}
  \caption{Linear fragment of the term assignment for \textbf{co-ND}}
  \label{fig:linear-ta}
\end{figure}
