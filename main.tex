...
transformations $\m{X,Y} : \func{F}X \otimes \func{F}Y \mto \func{F}(X
\times Y)$ and $\n{A,B} : \func{G}A \times \func{G}B \mto \func{G}(A
\otimes B)$, and maps $\m\top : \top \mto \func{F}1$ and $\n1 : 1 \mto
\func{G}\top$ subject to several coherence conditions; see
Definition~\ref{?}.  Furthermore, the functor $\func{F}$ is strong
which means that $\m{X,Y}$ and $\m{\top}$ are isomorphisms.  This
setup turns out to be one of the most beautiful models of
intuitionistic linear logic called an formally a LNL model due to
Benton \cite{Benton:1994}.  In fact, the linear modality of-course can
be defined by $!A = \func{F}(\func{G}(A))$ which defines a symmetric
...
$\cat{L'} : H \dashv \perp : \cat{C}$.  We will show that dual LNL
models are a simplification of dual linear categories as defined by
Bellin \cite{Bellin:2012} in much of the same way that adjoint models
are a simplification of linear categories.  In fact, we will define
Girard's exponential why-not by $? A = J(H(A))$, and hence, is the
...
immediately obtain a symmetric comonoidal monad on the left, and a
...
cocartesian coclosed, but this turns out to be where the matter meets
antimatter in such away that the category degenerates to a preorder.
That is, every homspace contains at most one morphism.  We recall this
proof here, which is due to Crolard \cite{Crolard:2001}. We need a
couple basic facts about cartesian closed categories with initial
objects.
\begin{lemma}
...
\end{proof}
\noindent
The following lemma is due to Joyal \cite{Crolard:2001}, and is key to
the next theorem.
\begin{lemma}[Joyal's]
  \label{lemma:joyals}
...
motivation for the current work.  Our ultimate goal is to enforce the
separation using linear logic, but through the power of linear logic
will be able to show that the separation is not far.
...
Compare this definition with that of Bellin's dual category from
...
\begin{definition}
  ...
  A \textbf{dual linear category}, $\cat{C}$, consists of the
  following data:
  \begin{enumerate}[i.]
  \item A symmetric monoidal coclosed category $(\cat{C}, \oplus, \perp, \colimp)$ with
  \item a symmetric co-monoidal monad $(\wn,\eta, \mu)$ on $\cat{C}$
    such that
  \begin{enumerate}[a.]
...
\subsubsection{Dual LNL Model Implies Dual Category}
...
\begin{corollary}
  ...
  Every dual LNL model is a dual category.
\end{corollary}
% subsubsection dual_lnl_model_implies_dual_category (end)

\subsubsection{Dual Category implies Dual LNL Model}
...
of a non-traditional style. We denote par by $[[A (+) B]]$, instead of
$[[A]] \parr [[B]]$.  Lastly, we denote linear coimplication by $[[A
    *- B]]$ to emphasize its duality with linear implication $[[A]]
\limp [[B]]$.  Each syntactic category of formulas contains the
respective functor from the dual LNL model, and thus, we should view
$H$ as the left adjoint to $J$.
...
\begin{figure}
  \begin{mdframed}
    \begin{mathpar}
      ...
      \DualLNLdruleCXXfR{} \and
      \DualLNLdruleCXXdL{} \and
      \DualLNLdruleCXXdR{} \and
