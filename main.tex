...
there is at most one morphism between any two objects.  However, this
degeneration does not occur when both logics are linear.  We propose
that these two worlds need to be separated, and then mixed in a
...
leave its analysis and correspond logic to a future paper.  Finally,
we give the definition of dual LNL logic and a term assignment
(Section~\ref{sec:dual_lnl_logic} and
...
Suppose $\cat{L}$ is a dual linear category, then the previous three
categories are related by a pair of adjunctions:
\begin{diagram}
...
\begin{lemma}
  ...
  If $\cat{C}$ is a dual linear category, then $\cat{C}^{\wn}$ has finite coproducts.
\end{lemma}
\begin{proof}
  We give a proof sketch of this result, because the proof is
  essentially by duality of Benton's corresponding proof for LNL
  models (see Lemma~9, \cite{Benton:1994}). Suppose $\cat{C}$ is a
...
\begin{definition}
  ...
  following:
  \begin{enumerate}[i.]
  \item a bi-linear category $(\cat{L},
  ...
  \item a symmetric monoidal adjunction $\cat{I} : F \dashv G$, and
  \item a symmetric comonoidal adjunction $\cat{L} : H \dashv J : \cat{C}$.
  \end{enumerate}
\end{definition}
